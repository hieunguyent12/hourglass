    ...
    DO: DeserializeOwned,
{
    ...
    let response = check_response_body::<DO, RE, TE>(&http_response)?;
...
fn check_response_body<DO, RE, TE>(
    http_response: &HttpResponse,
) -> Result<DO, RequestTokenError<RE, TE>>
where
    RE: Error + 'static,
    ...
    DO: DeserializeOwned,
{
    ...
    if http_response.body.is_empty() {
        ...
        let content: Result<DO, RequestTokenError<RE, TE>> = serde_path_to_error::deserialize(
            &mut serde_json::Deserializer::from_slice(response_body),
        )
        ...
        if let Ok(content) = content {
            ...
            let error = match serde_path_to_error::deserialize::<_, TE>(
                ...
                Ok(error) => RequestTokenError::ServerResponse(error),
                Err(error) => {
                    // the response can't be serialized into the ErrorResponse
                    if let Err(RequestTokenError::Parse(e, body)) = content {
                        return Err(RequestTokenError::Parse(e, body));
                    }
                    RequestTokenError::Parse(error, response_body.to_vec())
                }
            };
            return Err(error);
        }
